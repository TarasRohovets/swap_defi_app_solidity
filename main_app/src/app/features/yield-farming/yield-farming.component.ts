...
export class YieldFarmingComponent implements OnInit {
  ...
  async loadYieldFarming() {
   ...
   depoistTokens() {
    ...
    this.yieldFarmingSC.methods.depositTokens()
      .send({ value: convertedDepositAmountInput, from: this.account }).on('transactionHash', async (hash) => {
        await this.loadBlockChainData();
      });
  }

  withdrowTokens() {
    // let convertedWithdrowAmountInput = this.web3.utils
  }
